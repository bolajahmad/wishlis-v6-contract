...
mod wishlist {
    ...
    mod e2e_tests {
        ...
        async fn default_works(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
            // Given
            let mut constructor = WishlistRef::default();

            // When
            let contract = client
                .instantiate("wishlist", &ink_e2e::alice(), &mut constructor)
                .submit()
                .await
                .expect("instantiate failed");
            let call_builder = contract.call_builder::<Wishlist>();
        ...
        async fn it_works(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
            // Given
            let mut constructor = WishlistRef::new(false);
            let contract = client
                .instantiate("wishlist", &ink_e2e::bob(), &mut constructor)
                .submit()
                .await
                .expect("instantiate failed");
            let mut call_builder = contract.call_builder::<Wishlist>();
